...
public class CellSimModified{
	...
	public static void main(String[] args){
		...
		System.out.println("Enter the percentage of remaining cells that will be X's: ");
		int percentX=IO.readInt();
		
		System.out.println("Enter the percentage of remaining cells that will be Y's");
		int percentY=IO.readInt();
		...
		CellSimGUI csg=new CellSimGUI(n,1);
		...
		for(int m=1;m<=maxRounds;m++){
			...
			if(m==maxRounds&&boardSatisfied(tissue,threshold)==false){
				...
				printTissue(tissue);
				System.out.println("The board is: "+boardSatisfied(tissue,threshold));
				double u=(((totalX+totalO+totalY)-unsatisfied)*100)/(n*n);
	...
	**/
	public static void assignCellTypes(char[][] tissue, int percentBlank, int percentX, int percentY){
		...
		int numY=(int)Math.ceil((percentY/100.0)*(tissue.length*tissue.length-numBlank-numX));
		...
		for(int countY = 0; countY <numY; countY++ ){//this assigns random cells an X value
			int i=(int)Math.round((Math.random()*(tissue.length-1)));//.nextInt(tissue.length);
			int j=(int)Math.round((Math.random()*(tissue.length-1)));//.nextInt(tissue.length);
			if(tissue[i][j]!='X'&&tissue[i][j]!=' '&&tissue[i][j]!='Y'){//this makes sure the cell is not already filled
				tissue[i][j]=y;
			}
			else{
				countY--;//if the cell is filled, this makes sure that the count doesnt increment
			}
		...
		for(int i=0;i<tissue.length;i++){//this checks every cell to see if they have a value and fills
			...
			for(int j=0;j<tissue.length;j++){
				...
				if(tissue[i][j]!=space&&tissue[i][j]!=x&&tissue[i][j]!=y){
					tissue[i][j]=o;
